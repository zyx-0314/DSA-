...
void UpdatePatient(patient *Patients, int index) {
  if (index >= 0 && index < 10) {
    ...
    DataEntry("New First Name: ", Patients[index].fname);
    DataEntry("New Last Name: ", Patients[index].lname);
    DataEntry("New Age: ", Patients[index].age);
    DataEntry("New Disease: ", Patients[index].disease);
  } else {
    cout << "Invalid index.\n";
  }
}
